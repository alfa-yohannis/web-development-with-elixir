\chapter{Phoenix LiveView: Aplikasi Web Real-Time (bagian 1)}
\section{Pendahuluan}
...
\begin{itemize}
	\item \textbf{\texttt{use HiwiWeb, :live\_view}}: Mengaktifkan fungsionalitas LiveView pada modul ini, memungkinkan render dinamis dan pengelolaan event di sisi server.
...
\begin{lstlisting}[language=Elixir]
	...
	PubSub.broadcast(Hiwi.PubSub, "queue_counter:updates", {:number_update, new_number})
	{:noreply, new_number}
	end
	end
\end{lstlisting}
...
\subsection{Penjelasan Kode}
Modul \texttt{Hiwi.QueueCounter} ini memiliki beberapa fungsi yang masing-masing berperan dalam mengelola dan menyiarkan jumlah kasus yang ditangani. Berikut penjelasan dari fungsi-fungsi tersebut:
...
Modul \texttt{Hiwi.QueueCounter} merupakan bagian penting dari sistem yang membutuhkan pengelolaan state secara global dan penyiaran pembaruan ke berbagai komponen, memastikan bahwa semua pengguna dapat melihat perubahan secara langsung dan konsisten.

\section{Konfigurasi \texttt{Application} untuk Memulai \texttt{Hiwi.QueueCounter}}

Pada file \texttt{application.ex}, modul \texttt{Hiwi.Application} dikonfigurasi untuk memulai berbagai komponen utama aplikasi, termasuk GenServer \texttt{Hiwi.QueueCounter}. Penambahan \texttt{Hiwi.QueueCounter} sebagai child dalam aplikasi memastikan bahwa modul ini akan berjalan saat aplikasi dimulai, memberikan pengelolaan jumlah kasus yang ditangani secara real-time di seluruh sesi aplikasi.
...
\subsection{Penjelasan Komponen \texttt{Hiwi.QueueCounter} sebagai Child}
\texttt{Hiwi.QueueCounter} ditambahkan ke daftar \texttt{children}, sehingga GenServer ini akan diinisialisasi dan dijalankan saat aplikasi \texttt{Hiwi} dimulai. Fungsi \texttt{start\_link} dari modul \texttt{Hiwi.QueueCounter} akan secara otomatis dipanggil untuk menginisialisasi state awal, memungkinkan aplikasi untuk memantau jumlah kasus yang ditangani sejak awal aplikasi berjalan.

\subsection{Peran \texttt{Hiwi.QueueCounter} dalam Struktur Aplikasi}
Dengan menambahkan \texttt{Hiwi.QueueCounter} ke dalam daftar child, aplikasi mendapatkan akses langsung ke pengelolaan jumlah kasus melalui GenServer ini. Komponen \texttt{Hiwi.QueueCounter} akan tetap berjalan secara independen selama siklus hidup aplikasi, menyediakan API publik untuk mengakses dan memperbarui data kasus. Pengelolaan state di dalam \texttt{QueueCounter} terintegrasi dengan \texttt{Phoenix.PubSub}, memungkinkan pembaruan data disiarkan ke berbagai komponen dan sesi pengguna.

\subsection{Deskripsi Komponen Lain pada \texttt{Application}}
Selain \texttt{Hiwi.QueueCounter}, beberapa komponen lain yang disertakan dalam daftar \texttt{children} adalah sebagai berikut:

\begin{itemize}
	\item \textbf{\texttt{HiwiWeb.Telemetry}}: Memulai layanan telemetry untuk pemantauan performa aplikasi.
	\item \textbf{\texttt{Hiwi.Repo}}: Menghubungkan aplikasi ke database menggunakan \texttt{Ecto.Repo}.
	...
	\item \textbf{\texttt{Hiwi.Finch}}: Memulai client HTTP \texttt{Finch} yang dapat digunakan untuk mengirim email atau request HTTP lainnya.
	\item \textbf{\texttt{HiwiWeb.Endpoint}}: Memulai \texttt{Endpoint} sebagai entry terakhir untuk menangani permintaan HTTP dan WebSocket, yang biasanya menjadi komponen terakhir dalam daftar \texttt{children}.
\end{itemize}

\subsection{Pentingnya \texttt{Hiwi.QueueCounter} dalam Daftar Child}
Dengan mengonfigurasi \texttt{Hiwi.QueueCounter} sebagai bagian dari struktur aplikasi, aplikasi memiliki kontrol yang kuat dan konsisten terhadap pengelolaan jumlah kasus yang ditangani. Selain itu, integrasi \texttt{QueueCounter} dengan \texttt{Phoenix.PubSub} memungkinkan sinkronisasi data secara real-time di seluruh komponen aplikasi. Hal ini memberikan fleksibilitas dalam penanganan data real-time dan meningkatkan performa serta konsistensi data pada aplikasi berbasis Phoenix Framework ini.

Kode di atas menunjukkan bagaimana setiap komponen diatur untuk bekerja secara harmonis dalam aplikasi, dengan \texttt{Hiwi.QueueCounter} sebagai elemen penting untuk pengelolaan state dan sinkronisasi data.


\section{Implementasi Modul \texttt{ModifiableLive} untuk Pengelolaan Kasus dengan \texttt{LiveView}}

Modul \texttt{HiwiWeb.ModifiableLive} menggunakan \texttt{Phoenix LiveView} untuk menampilkan dan mengelola kasus secara real-time. Modul ini berinteraksi dengan \texttt{QueueCounter} sebagai GenServer untuk mengelola jumlah kasus yang ditangani, dan memanfaatkan \texttt{Phoenix.PubSub} untuk memperbarui data secara sinkron pada tampilan pengguna. 
...
\subsection{Pentingnya Interaksi dengan \texttt{Hiwi.QueueCounter}}
Interaksi modul \texttt{ModifiableLive} dengan \texttt{Hiwi.QueueCounter} memastikan bahwa setiap pembaruan jumlah kasus dapat diakses oleh seluruh pengguna yang melihat data ini. Penggunaan PubSub memungkinkan sinkronisasi antar komponen dan halaman secara real-time, memberikan pengalaman pengguna yang lebih responsif dan konsisten tanpa perlu penyegaran halaman.
...
\begin{itemize}
	...
	\item Berlangganan ke topik \texttt{"queue\_counter:updates"} di \texttt{Hiwi.PubSub}, memungkinkan tampilan untuk menerima pembaruan nomor antrian secara real-time dari \texttt{QueueCounter}.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \texttt{<.live\_component module={HiwiWeb.QueueCounterComponent} ...>}: Kode tersebut meng-\textit{embed} Komponen \texttt{QueueCounterComponent} ke dalam tampilan tsb. Komponen ini menerima atribut \texttt{queue\_prefix} dan \texttt{current\_number} yang diassign dari variabel \texttt{@queue\_prefix} dan \texttt{@current\_number}, sehingga nomor antrian dapat dirender secara real-time. Dengan \texttt{id="queue\_counter"}, komponen ini memiliki identitas unik dalam tampilan LiveView.
	\end{itemize}
\end{itemize}
...
\section{Ringkasan}
Bab ini memperkenalkan Phoenix LiveView sebagai pendekatan server-rendered yang mampu menghadirkan antarmuka web real-time tanpa ketergantungan pada framework JavaScript di sisi klien. Dengan memanfaatkan proses ringan Elixir dan konkurensi BEAM, LiveView mengelola siklus hidup tampilan melalui callback seperti \texttt{mount/3}, \texttt{render/1}, dan mekanisme diff tracking untuk hanya mengirim perubahan HTML yang diperlukan. Manajemen state terpusat di \texttt{socket.assigns}, mendukung pembedaan antara state persisten dan ephemeral, sementara penanganan event \texttt{handle\_event/3}, integrasi PubSub, manajemen sesi, dan perlindungan CSRF memastikan interaktivitas yang aman, konsisten, dan efisien. Bab ini juga menyoroti keunggulan dan keterbatasan LiveView, termasuk kesederhanaan pengembangan, efisiensi server-side rendering, kebutuhan WebSocket, serta potensi beban server pada aplikasi dengan lalu lintas tinggi.

Bagian praktik menunjukkan bagaimana konsep tersebut diterapkan dalam studi kasus antrian dan termostat. Contoh \texttt{ThermostatLive} menggambarkan alur dasar \texttt{mount/3}, \texttt{render/1}, dan \texttt{handle\_event/3} untuk pembaruan suhu secara real-time. Untuk skenario antrian, bab ini menghubungkan rute LiveView di \texttt{router.ex}, kontrol akses berbasis pengguna di \texttt{index.html.heex}, serta penggunaan \texttt{Hiwi.QueueCounter} sebagai \texttt{GenServer} yang disiarkan melalui \texttt{Phoenix.PubSub}. Modul \texttt{ModifiableLive} dan \texttt{DisplayOnlyLive} menunjukkan bagaimana pembaruan nomor antrian dapat dimodifikasi dan ditampilkan secara sinkron, sementara \texttt{QueueCounterComponent} dan \texttt{display\_only\_live.html.heex} menjelaskan desain antarmuka read-only berukuran besar untuk layar publik. Secara keseluruhan, bab ini menggabungkan konsep arsitektural dan contoh kode konkret untuk membangun aplikasi antrian real-time berbasis Phoenix LiveView.
